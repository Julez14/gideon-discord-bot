...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("births")
    .setDescription("Returns the births that occurred on a certain date")
  ...
  async execute(interaction) {
    ...
    if (!interaction.options.getString("year")) {
      console.log(`Searching for all births on ${date}`);
      ...
      let fieldCounter = 0;
      for (let i = 0; i < data.Births.length - 1; i++) {
        if (fieldCounter < 25) {
          newEmbed1.addFields({
            name: data.Births[i].year,
            value: data.Births[i].text,
          });
    ...
    console.log(`Searching for births on ${date}/${chosenYear}`);
    ...
    let yearCheck = "";
    for (let i = 0; i < data.Births.length - 1; i++) {
      yearCheck = data.Births[i].year.split(" ").join("");
