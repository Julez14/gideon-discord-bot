...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("events")
    .setDescription("Returns the events that occurred on a certain date")
  ...
  async execute(interaction) {
    ...
    if (!interaction.options.getString("year")) {
      console.log(`Searching for all events on ${date}`);
      ...
      for (let i = 0; i < data.Events.length - 1; i++) {
        if (fieldCounter < 25) {
          newEmbed1.addFields({
            name: data.Events[i].year,
            value: data.Events[i].text,
          });
    ...
    console.log(`Searching for events on ${date}/${chosenYear}`);
    ...
    let yearCheck = "";
    for (let i = 0; i < data.Events.length - 1; i++) {
      yearCheck = data.Events[i].year.split(" ").join("");
